...
public class LargestPiece {
    ...
    public static int max1s(String[] edge, boolean[][] visited, int x, int y, int n) {
        int[] X_dir = { 0, 1, -1, 0 }; // we are moving in all 4 directions, top, right, down ,left, these are
                                       // coordinates for these only
 ...
 * consisting of '0's and '1's . But Gary wants the biggest piece of '1's and no
 * '0's . A piece of cake is defined as a part which consist of only '1's, and
 * all '1's share an edge with each other on the cake. Given the size of cake N
 * and the cake, can you find the count of '1's in the biggest piece of '1's for
 * Gary ?
 * Input Format :
